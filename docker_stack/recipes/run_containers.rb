...
node[:my_apps].each do |name, image|  
  script "run_app_#{name}_container" do
    ...
    user "root"
    code <<-EOH
      docker run -d -p 3000:3000 -e RAILS_ENV=production -e SECRET_KEY_BASE=#{ENV['SECRET_KEY_BASE']} -e DB_HOST=#{ENV['DB_HOST']} -e DB_USER=#{ENV['DB_USER']} -e DB_PASSWORD=#{ENV['DB_PASSWORD']} --name=#{name} #{image}
    EOH
  end
end
